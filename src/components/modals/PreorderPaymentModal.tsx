...
export default function PreorderPaymentModal({
  ...
  const [payments, setPayments] = useState<Payment[]>([
    { type: "cash", amount: "", checkNumber: "" },
  ]);
  const handlePayments = async (type: "cash" | "check" | "", checkNumber?: string) => {
    if(type === "cash"){
      setPayments([{type: type, amount: preorder.total, checkNumber: ""}]);
      }
    if(type === "check"){
      setPayments([{type: type, amount: preorder.total, checkNumber: checkNumber ?? ""}]);
    }
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              handlePayments(e.target.value as any, "");
            }}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                handlePayments("check", e.target.value);
              }}
              className="bg-(--tertiary) rounded-xl shadow-xl h-10 p-2 w-full"
            />
          </div>
