...
export default function AddPaymentTermWizard({ onSuccess }: { onSuccess?: () => void }){
    const [step, setStep] = useState(1);
  const steps = ["Payment Term Info"];
  ...
  async function handleConfirm () {
    ...
    try{
      const payload: any = {
        name: form.name,
        days: form.days,
        dueOnReceipt: form.dueOnReceipt,
      };
