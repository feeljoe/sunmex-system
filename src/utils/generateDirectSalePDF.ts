...
export function generateDirectSalePDF(directSale: any) {
  ...
  const filteredProducts = (directSale.products || []).filter((p: any) => {
  ...
  autoTable(doc, {
    ...
    body: [
      [
        directSale.number || "NA",
        new Date(directSale.createdAt).toLocaleDateString(),
      ],
    ],
    tableWidth: 150,
  ...
  autoTable(doc, {
    ...
    theme: "grid",
    head: [[ "Sold To"]],
    body: [[ directSale.client.clientName],],
    styles: { fontSize: 8, halign: "center"},
    headStyles: {
      fillColor: [0, 102, 204], // RGB color (blue)
      textColor: 255,           // white text
  ...
  autoTable(doc, {
    ...
    theme: "grid",
    head: [[ "Sales Person"]],
    body: [[ `${directSale.createdBy.firstName} ${directSale.createdBy.lastName}`],],
    styles: { fontSize: 8, halign: "center"},
    headStyles: {
      fillColor: [0, 102, 204], // RGB color (blue)
      textColor: 255,           // white text
  ...
  autoTable(doc, {
    ...
    theme: "grid",
    head: [[ "Terms"]],
    body: [[ directSale.client.paymentTerm.name],],
    styles: { fontSize: 8, halign: "center"},
    headStyles: {
      fillColor: [0, 102, 204], // RGB color (blue)
      textColor: 255,           // white text
...
if (directSale.status === "cancelled") {
  ...
  cursorY += 15;
} else if (directSale.status === "delivered") {
  doc.setFontSize(12);
  doc.text("DELIVERY CONFIRMATION", 40, cursorY + 10);
  if(directSale.status === "delivered") {
    doc.text(
  ...
  if(directSale.cancelReason) {
    doc.text(
      `Reason for Cancellation: ${directSale.cancelReason}`,
      40,
      cursorY
    );
    cursorY += 25;
  ...
  autoTable(doc, {
    ...
    body: sortedProducts.map((p: any) => {
      ...
      const price = p.actualCost ?? p.unitPrice ?? 0;
...
// SIGNATURE (LEFT SIDE)
// -------------------
if (directSale.status === "delivered" && directSale.signature) {
  const sigX = 40;
  ...
  doc.setFontSize(9);
  doc.text(directSale.client.clientName, sigX, sigY + 65);
}
